...
write.table(fcwt.sec,file="sec.max-all-wt.txt",sep='\t',quote=F,row.names=F)
...
aa0 <- subset(a4,minus>0.7&minus<1) # 61 genes
aa1 <- subset(a4,minus>1&minus<1.5) # 47 genes
aa2 <- subset(a4,minus>1.5&minus<2) # 11 genes
...
```
## 6.3 Using flat slope method to find the definite and direct Pho4 target candidates
```{r}
...
## 7.1 visualize the most significant log2FC via max log2FC method
```{r}
## 8 FC difference
...
```
# 7.2 visualize mildly induced DEGs according to clustered heatmap of lfc=log2(4)&P<0.05 of wt DEGs, FC value was set to be saturated to 4.
```{r}
