...
dif.20m<-cbind(genesymbol.20m,dif.20m)
dif.30m<-cbind(genesymbol.30m,dif.30m)
dif.45m<-cbind(genesymbol.45m,dif.45m)
dif.60m<-cbind(genesymbol.60m,dif.60m)
dif.90m<-cbind(genesymbol.90m,dif.90m)
dif.120m<-cbind(genesymbol.120m,dif.120m)
dif.150m<-cbind(genesymbol.150m,dif.150m)
dif.180m<-cbind(genesymbol.180m,dif.180m)
dif.240m<-cbind(genesymbol.240m,dif.240m)
...
```{r}
##6.screen each group's diff probeid
...
---
```{r}
##8. plot pheatmap of ORGs and DEGs in early or late group
## get the annotation file with real gene names, and name the txt file as probeid_genesymbol,go out R studio, use 4.probeid to genesymbol.perl to convert all the probe IDs in the WT-FDR-adj.p0.05.exprs.txt into genenames, and discard the uncharacterized ORFs. We got 194 DEGs in the new list genesymbol.FDR0.05.exprs.txt. Then we use them to seperately present the kinetics of all up- and down- regulated ORFs and genes and visualize them in heatmaps.
...
DEG_exp<-read.table("WT-FDR-adj.p0.05.exprs.txt",
                    sep='\t',header=T,row.names=1)
pdf(file="pheatmap-wt.pdf",width=50,height=25)
pheatmap(DEG_exp,
...
DEG_exp<-read.table("genesymbol.FDR0.05.exprs.txt",
                    sep='\t',header=T,row.names=1)
pdf(file="pheatmap-wtgenes.pdf",width=50,height=25)
pheatmap(DEG_exp,
         color=colorRampPalette(c("blue","white","orange"))(100),
         fontsize_row=4,
         fontsize_col=4,scale="row",
         border_color=NA,cluster_col = FALSE)
dev.off()
```
