...
cpm.n <- cbind(cpm.n,genename)
write.table(cpm.n,file="TMMnorm.txt",row.names=FALSE,sep="\t",quote=F)
```

```{r}
5## create design matrix
eset <- read.table("TMMnorm.txt",header = TRUE, sep="\t") ## why we need sep="\t"
condition=factor(c(rep("Pre",2),rep("20min",2)))
design<-model.matrix(~-1+condition) ##Do not understand ~-1+condition 
colnames(design)<-c("Pre","20min")
```
