...
binterc.genes <- del4.fc[binterc,]
### B∩C & ((Complementary Set of B)∩C)
maxn <- function(n) function(x) order(x, decreasing = TRUE)[n]
binterc.genes[,"sec.max"] <- apply(binterc.genes[,2:10],1,function(x) x[maxn(2)(x)])
binterc.up <- which(binterc.genes$sec.max>=1.8,binterc.genes$sec.max)
binterc.upge <- binterc.genes[binterc.up,] # 114 genes upregulated in the B∩C; 41 super sure
write.table(binterc.upge,file="up-BinterC.txt",sep='\t',quote=F,row.names=T)
onlyc <- del4.fc[!del4.fc[,1] %in% del80.fc[,1],]
onlyc[,"sec.max"] <- apply(onlyc[,2:10],1,function(x) x[maxn(2)(x)])
onlyc.up <- which(onlyc$sec.max>=1.8,onlyc$sec.max)
onlyc.upge <- onlyc[onlyc.up,] #  genes in the onlyCwoB (Pho4 inde PSR genes), includes genes only activated in del4 no pi groups but not activated in wt no Pi groups
...
wt.fc.up <- which(wt.fc$sec.max>=1.8,wt.fc$sec.max)
wt.fc.upge <- wt.fc[wt.fc.up,]
onlycmod <- wt.fc.upge[wt.fc.upge[,1.8] %in% onlyc.upge[,1],] # log2=1.8 376 genes out of 525 genes were also activated in wt no Pi group; log2=1 917 genes
...
```
## 7.4 the overall Pho4 targets (PSR+other responses)
```{r}
### A∪B-((Complementary Set of B)∩C)∩A log2=1.8 252+16-=268 genes; log2=1 252+23=275 genes
del80.fc.upge <- subset(del80.fc,del80.fc$logFC>=1.8) # 60 genes; log2=1 149 genes
del80.pho4 <- del80.fc.upge[!del80.fc.upge[,1] %in% onlycmod[,1],] # 60 genes; log2=1 149 genes
wt.pho4 <- wt.fc.upge[!wt.fc.upge[,1] %in% onlycmod[,1],] # 252 genes; log2=1 363 genes
test1 <- del80.fc.upge[!del80.pho4[,1] %in% wt.pho4[,1],] # 15 genes; log2=1 23 genes
### Pho4 overall targets = genes from test1&wt.pho4
### Pho4 targets log2=1
