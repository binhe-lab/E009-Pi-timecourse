...
wt.fc <- dif044[,1:10]
del80.fc <- dif077[,1:2]; colnames(del80.fc) <- c("genes","pho80pho4vspho80")
del4.fc <- dif066[,1:10]
maxn <- function(n) function(x) order(x, decreasing = TRUE)[n]
wt.fc[,"no.pi.wt.sec.max"] <- apply(wt.fc[,2:10],1,function(x) x[maxn(2)(x)])
del4.fc[,"no.pi.del4.sec.max"] <- apply(del4.fc[,2:10],1,function(x) x[maxn(2)(x)])
wt.fc <- subset(wt.fc,wt.fc$no.pi.wt.sec.max>=1.5) # alter the threshold here
del4.fc <- subset(del4.fc,del4.fc$no.pi.del4.sec.max>=1.5) # alter the threshold here
del80.fc <- subset(del80.fc,del80.fc$pho80pho4vspho80>=1.5) # alter the threshold here
### B∩C 
binterc.upge <- del4.fc[del4.fc[,1] %in% del80.fc[,1],] 
binterc.or <- binterc.upge[with(binterc.upge,order(genes,decreasing = FALSE)),]
binterc2 <- del80.fc[del80.fc[,1] %in% del4.fc[,1],]
binterc2.or <- binterc2[with(binterc2,order(genes,decreasing = FALSE)),]
binterc.all <- cbind(binterc2.or,binterc.or[,2:11]) # log2=0.85 150 genes, log2=1 120 genes; log2=1.5 58 genes
write.table(binterc.all,file="up-BinterC-log2=1.5.txt",sep='\t',quote=F,row.names=T) #add -log2=numbers for different fc threshold
### A'∩(B∩C) (Co+ of Pho4 targets) (all)
reduntar <- binterc.upge[!binterc.upge[,1] %in% wt.fc[,1],]
###B'∩C
onlyc.upge <- del4.fc[!del4.fc[,1] %in% del80.fc[,1],]
write.table(onlyc.upge,file="up-onlyCwoB.txt",sep='\t',quote=F,row.names=T)
### (B'∩C)∩A
onlycmod <- wt.fc[wt.fc[,1] %in% onlyc.upge[,1],] # log2=0.85 1094 genes, log2=1.8 376 genes out of 525 genes were also activated in wt no Pi group; log2=1 944 genes
write.table(onlycmod,file="up-Pho4indetargets.txt",sep='\t',quote=F,row.names=T)
### A'∩B & A∩B
bnotina <- del80.fc[!del80.fc[,1] %in% wt.fc[,1],] # log2=0.85 25 genes, log2=1 23 genes, log2=1.5 17 genes 
bina <- del80.fc[del80.fc[,1] %in% wt.fc[,1],]
bina.or <- bina[with(bina,order(genes,decreasing = FALSE)),]
bina2 <- wt.fc[wt.fc[,1] %in% bina.or[,1],]
bina2.or <- bina2[with(bina2,order(genes,decreasing = FALSE)),]
bina.all <- cbind(bina.or,bina2.or[,2:11])  # log2=0.85 157 genes, log2=1 126 genes, log2=1.5 66 genes
write.table(bina.all,file="up-bina-log2=1.5.txt",sep='\t',quote=F,row.names=T)
write.table(bnotina,file="up-bnotina-log2=1.5.txt",sep='\t',quote=F,row.names=T) #add -log2=numbers for different fc threshold
### A'∩C
cnotina <- del4.fc[!del4.fc[,1] %in% wt.fc[,1],] # log2=0.85 ? genes, log2=1 ? genes, log2=1.5 185 genes
write.table(cnotina,file="up-cnotina-log2=1.5.txt",sep='\t',quote=F,row.names=T)
### Pho4 definite targets A - (A∩C)∩B' 
cnotinb <- del4.fc[!del4.fc[,1] %in% del80.fc[,1],]
cnbina <- cnotinb[cnotinb[,1] %in% wt.fc[,1],]
pho4def.tar <- wt.fc[!wt.fc[,1] %in% cnbina[,1],] #log2=0.85 log2=1 336 genes log2=1.5 235 genes
### compare the gene list of A and 79 genes in Bin's list
bin79 <- fread("../sandbox/elife-25157-fig5-data2-v2.csv", check.names = FALSE, stringsAsFactors = FALSE) 
ain79 <- wt.fc[wt.fc$genes %in% bin79$GeneID,] #log2=0.85 65 genes, log2=1 64, log2=1.5 61 genes found in a were also in 79 genes list
### Output subsets of genes
#write.table(onlyc.upge,file="up-onlyCwoB.txt",sep='\t',quote=F,row.names=T)
#write.table(bina.all,file="up-bina-log2=1.5.txt",sep='\t',quote=F,row.names=T)
#write.table(binterc.all,file="up-BinterC-log2=1.5.txt",sep='\t',quote=F,row.names=T) #add -log2=numbers for different fc threshold
#write.table(bnotina,file="up-bnotina-log2=1.5.txt",sep='\t',quote=F,row.names=T) #add -log2=numbers for different fc threshold
#write.table(cnotina,file="up-cnotina-log2=1.5.txt",sep='\t',quote=F,row.names=T)
#write.table(onlycmod,file="up-Pho4indetargets.txt",sep='\t',quote=F,row.names=T)
#write.table(pho4def.tar,file="up-pho4deftar-log2=0.85txt",sep='\t',quote=F,row.names=T)
```
...
```{r}
## Msn2/4 and its downstream targets
## Msn2 & Msn4 & Ap1(ScYap1) & Skn7 & cta1 & Tsa1
myGenePlot(genes = c("CAGL0F05995g","CAGL0M13189g","CAGL0H04631g","CAGL0F09097g","CAGL0K10868g","CAGL0K06259g"))
## Msn2 & Msn4 & GLK1 & SSA3(could be independent in some case) & TPS2 & TPS1 
myGenePlot(genes = c("CAGL0F05995g","CAGL0M13189g","CAGL0F00605g","CAGL0G03289g","CAGL0J09812g","CAGL0G05335g","CAGL0J09812g"))
## Msn2 & Msn4 & HSP12 & ALD5 & SOD2 & PGM2 & HXK1 
myGenePlot(genes = c("CAGL0F05995g","CAGL0M13189g","CAGL0J04202g","CAGL0J03212g","CAGL0E04356g","CAGL0K03421g","CAGL0A04829g"))
## Msn2 & Msn4 & HSP12 & PGM2 
myGenePlot(genes = c("CAGL0F05995g","CAGL0M13189g","CAGL0J04202g","CAGL0K03421g"))

## A'∩B log2=1 
myGenePlot(genes = c("CAGL0C00110g","CAGL0E01705g","CAGL0L08547g","CAGL0C05643g","CAGL0E06292g","CAGL0L03267g","CAGL0J06028g","CAGL0L03157g","CAGL0E05632g","CAGL0K05390g","CAGL0I00286g","CAGL0M00880g"))
myGenePlot(genes = c("CAGL0I10747g","CAGL0I10791g","CAGL0E03850g","CAGL0M12881g","CAGL0L00583g","CAGL0F05863g","CAGL0D01958g","CAGL0K08338g","CAGL0J03212g","CAGL0G01320g","CAGL0G04763g"))
## A'∩C 
myGenePlot(genes = c("CAGL0A02321g","CAGL0M01166g","CAGL0L00671g","CAGL0G04081g","CAGL0E03630g","CAGL0C05071g","CAGL0I09086g","CAGL0M11308g","CAGL0K08844g","CAGL0L10362g","CAGL0D00286g","CAGL0J06358g","CAGL0J03916g","CAGL0M08866g","CAGL0L10043g","CAGL0L12606g","CAGL0L01089g","CAGL0G01122g","CAGL0M04499g","CAGL0K02563g","CAGL0K01507g"))
myGenePlot(genes = c("CAGL0K07876g","CAGL0M00968g","CAGL0L06358g","CAGL0H03003g","CAGL0M00616g","CAGL0L06666g","CAGL0C02519g","CAGL0H08129g","CAGL0D00770g","CAGL0A02838g","CAGL0J11484g","CAGL0K10714g","CAGL0H03795g","CAGL0I05632g","CAGL0I08613g","CAGL0F06875g","CAGL0I00836g","CAGL0E01023g","CAGL0L05676g"))
