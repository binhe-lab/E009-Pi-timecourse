...
wt.fc <- subset(wt.fc,wt.fc$sec.max>=1)
del4.fc <- subset(del4.fc,del4.fc$sec.max>=1)
del80.fc <- subset(del80.fc,del80.fc$logFC>=1)
### B∩C 
binterc <- match(del80.fc[,1],del4.fc[,1]); binterc <- sort(binterc)
binterc.upge <- del4.fc[binterc,] # log2=0.85, 150 genes, log2=1 120 genes; 41 super sure
write.table(binterc.upge,file="up-BinterC.txt",sep='\t',quote=F,row.names=T)
### A'∩(B∩C) (Co+ of Pho4 targets) (all)
reduntar <- binterc.upge[!binterc.upge[,1] %in% wt.fc[,1],] #log2=0.85 9 genes, log2=1.8 10 genes; log2=1 9 genes; log2=0.85 18 gene
###B'∩C
...
### A'∩B & A∩B
bnotina <- del80.fc[!del80.fc[,1] %in% wt.fc[,1],] #log2=0.85 25 genes, log2=1 23 genes, 
bina <- del80.fc[del80.fc[,1] %in% wt.fc[,1],]; #log2=0.85 157 genes, log2=1 126 genes,
bina2 <- match(wt.fc[,1],bina[,1]); bina3 <- bina2[with(bina2,order(genes,logFC)),]
bina3 <- wt.fc[bina2,]
write.table(bina,file="up-bina.txt",sep='\t',quote=F,row.names=T)
write.table(bnotina,file="up-bnotina.txt",sep='\t',quote=F,row.names=T)
...
ain79 <- wt.fc[wt.fc$genes %in% bin79$GeneID,] #log2=1 64 genes found in a were also in 79 genes list
```
